...
contract PuppyContract {
    //Events
    event PuppyAdded(string microchipId, address petAddress);
    event VetAdded(address vetAddress);
    ...
    mapping(uint256 => PuppyLibrary.Transfer) public transfers;
    ...
    function registerVeterinarian(
        ...
        require(
            !veterinarians[msg.sender].isRegistered,
            "Account already registered as Veterinarian"
        );
        //check msg.sender not existing in owners
        require(
            !owners[msg.sender].isRegistered,
            "Account already registered as Owner"
        );
        //check msg.sender not existing in puppies
        require(
            !puppies[msg.sender].isRegistered,
            "Account already registered as Puppy"
        );
        ...
        emit VetAdded(msg.sender);
    }

    //register address as Puppy
    function registerPuppy(
        PuppyLibrary.PuppyType _type,
        ...
        emit PuppyAdded(_microchipId, msg.sender);
    }

    //register sender as Owner
    function registerOwner(
        PuppyLibrary.OwnerType _ownerType,
        string memory _name,
        string memory _surname,
        //string memory _birthDate,
        //string memory _homeAddress,
    ...
    //get Owner Data
    function getOwnerData(address _ownerAddress)
        public
        view
        returns (PuppyLibrary.Owner memory)
    {
        return owners[_ownerAddress];
    }

    //get Puppy Data
    function getPuppyData(address _puppyAddress)
        public
        view
        returns (PuppyLibrary.Puppy memory)
    {
        return puppies[_puppyAddress];
    }

    //get Vaccine Data
    function getVaccineData(address _puppyAddress)
        public
        view
        returns (PuppyLibrary.Vaccine[] memory)
    {
        return puppies[_puppyAddress].vaccines;
    }

    //get Veterinarian Data
    function getVeterinarianData(address _veterinarianAddress)
        public
        view
        returns (PuppyLibrary.Veterinarian memory)
    {
    ...
    function addVaccine(
        address _puppyAddress,
        string memory _vaccineBatch,
        string memory _date,
        string memory _vaccineType
    ) public onlyVeterinarian {
        PuppyLibrary.Vaccine memory v =
            PuppyLibrary.Vaccine(_vaccineBatch, _date, _vaccineType);
    ...
    function transferOwner(address _puppyAddress, address newOwner)
        public
        onlyPuppyOwner(_puppyAddress)
    {
        ...
        for (uint256 i = 0; i < owners[msg.sender].puppies.length - 1; i++) {
