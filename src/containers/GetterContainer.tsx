...
const GetterContainer = (props: any) => {
  ...
  const [transferOwnershipFormData, setTransferOwnershipFormData] = useState<
    any
  >({});
  ...
  useEffect(() => {
    ...
    console.log(
      "[++++ Init +++++] - transferOwnershipFormData: ",
  ...
  return (
    <GetterComponent
      ...
      onSubmitVaccine={() =>
        props.addVaccine(
          config,
          _.mapValues(formData, (o) => {
            return o.value;
          })
        )
      }
      transferPuppyOwnership={() =>
        props.transferPuppyOwnership(
          config,
          _.mapValues(transferOwnershipFormData, (o) => {
            return o.value;
          })
        )
      }
...
const mapDispatchToProps = (dispatch: any) => ({
  getVeterinarian: (config: any, data: string) =>
    dispatch(getVeterinarian(config, data)),
  ...
  transferPuppyOwnership: (config: any, data: any) =>
    dispatch(transferPuppyOwnership(config, data)),
});

const mapStateToProps = (state: any) => ({
  loading: state.contractReducer.loading,
  veterinarian: state.contractReducer.veterinarian,
  owner: state.contractReducer.owner,
  puppy: state.contractReducer.puppy,
  getter: state.contractReducer.getter,
  toast: state.toastReducer,
});

export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(injectIntl(GetterContainer))
);
